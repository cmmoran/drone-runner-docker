...
type (
	...
	Step struct {
		...
		OutputSettings map[string]stepoutput.OutputRef `json:"output_settings,omitempty"`
...
func (s *Step) Clone() runtime.Step {
	...
	*dst = *s
	...
	if len(s.OutputSettings) != 0 {
		dst.OutputSettings = map[string]stepoutput.OutputRef{}
		for key, value := range s.OutputSettings {
			dst.OutputSettings[key] = value
		}
	}
	return dst
}
